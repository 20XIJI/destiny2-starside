...
记录按页面、分节分组，按出现顺序归属前面最近的页面与分节：

| 记录 | 形状 |
|---|---|
| 页面，每页一条 | `{"u":路径,"t":标题,"d":描述}`，其后是这一页的分节与条目 |
| 分节，每节一条 | `{"a":锚点,"l":分节}`，其后是这一节的条目 |
| 条目 | `[名称, 全文]`，全文里第一个 `¶` 代写名称；全文末尾接英文名 |
| 条目，全文与前一个同名条目相同 | `[名称]` |

站上不压缩传输，索引的字节原样下到读者那里，所以页面与分节不在每个条目上重复，
名称不在全文里再写一遍，同名条目逐字相同的全文只写一次（异域武器、异域护甲的
详解页与刷取清单页各有一百多条相同）。首页 `home.js` 的 `unpack()` 把它展开回
每条带齐 `u` `a` `l` `n` `x` 的记录，匹配、排序与渲染只认展开后的形状。
全文里本来就有 `¶` 时 `build-search.py` 中止：展开会把那一处换成名称。

英文名接在全文末尾，从 `data/` 记录的 `i18n.en.name` 取——那一份是 manifest 蒸馏来的，
4302 个条目里 2589 条接上了。**只进全文那一格，不上页面**：站点是中文站，英文名在正文里
出现一次都会破坏版式。代价 33 KB，索引空闲预取、不进首屏。`check_quality.cjs` 用
`home.js` 的 `unpack()` 展开索引后钉着这一条——掉了的症状是「搜中文照样能搜到」，
没有人会发现。同一条测试也查展开结果：每个条目挂得上页面与分节，全文里不剩 `¶`。
...
索引不进首屏，由 `home.js` 注入一个 `<script>` 取回。它 1167 KB，站上不压缩传输，
