...
| 条目，每条一条 | `u` 路径、`a` 锚点、`l` 分节、`n` 名称、`x` 全文（末尾接英文名） |

英文名接在 `x` 末尾，从 `data/ 的记录（i18n.en.name）` 的 `i18n.en.name` 取——那一份是
manifest 蒸馏来的，18840/19356 条本来就带着英文，4282 个条目里 1940 条接上了。
**只进全文那一格，不上页面**：站点是中文站，英文名在正文里出现一次都会破坏版式。
代价 25 KB gz（354 → 379），索引空闲预取、不进首屏。`check_quality.py` 的
`Generated` 钉着这一条——掉了的症状是「搜中文照样能搜到」，没有人会发现。
