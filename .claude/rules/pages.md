...
| `首屏记录：` | 可选，只把前 N 条记录留在 `index.html`，其余移进同目录的 `rest.gz`（gzip）与 `rest.html`（原样），页面上的内联脚本取回来插回原位（`shell.split_rest`）。给托管不压缩时上千 KB 的长页用，现有购物清单三页写 3。不开 JavaScript 的读者只看得到首屏那几条 |
| `此刻：是` | 可选，见下 |
| `说明排印：是` | 可选，每行末格（说明）按空行分段，每段交给 `markup.typeset()`：首行以冒号收尾的触发条件包进 `.cond`，素字数值包进 `<b>`。现在只有护甲模组页写 |
| `图表：是` | 可选，把表画成折线图，见下 |
| `标注：` | 可选，`列名 值 值 …`，图上默认标出的几个点，可写多行；要配合 `图表：是` |
| `默认曲线：` | 可选，`列名、列名`，加载时只画这几条曲线，其余由图例打开；要配合 `图表：是` |

`导航`／`此刻`／`图表`／`说明排印` 是布尔键，只认「是」。写别的值（包括「否」）当场报错：不需要就把整行删掉，不留一行写着「否」的开关。
...
| `![](icons/xxx.webp)` | `<img src alt="" width height>`，宽高从文件头现读，文件名须是内容的 md5 前 10 位；`assets/icons/` 下按主键取的官方图用 Bungie 的图名，不复核 |
...
表格只写主键的源稿，行的内容在 `data/` 的记录上：改这些页的文案改记录，不改源稿。`convert-doc.py` 见到「列：」行即交给 `tools/rows.py` 的 `expand()`，展开成 `| 表头 |` 表，其后照常走 `render_table()` 与 `check()`。

表区是「列：A | B | C」那一行加紧接着的连续非空行，遇空行或 `##` 结束。一行写 `主键 [主键…]  行标题`：第一枚是行首，其余是它盖住的 hash，两个空格之后是行标题。`== 组名 ==` 是横幅行。
...
| 异域两页的「说明」 | 本体第一段，接各宿主的说明，再接本体其余段落。宿主只收只被这一行够得着、且说明不在别页某一行上的那几枚，判据见 `rows.own_hosts()` |
| 矩阵表的格子 | `references/pages/<页>.json` 的 `matrix`，按分节名（或「分节 · 横幅名」）与行首主键找 |

两种子行紧跟在本行之后画，源稿里不写：技能记录的 `enhanced` 一条一行，第一列是星相名；框架记录的 `weaponTypes` 一条一行，第一列是「框架名（枪型、枪型）」。框架记录根上没有说明时只画子行。

记录上取不出来的格子留空、记进 `rows.PROBLEMS`，全部页面写完后 `convert-doc.py` 一次报出并中止。缺什么补什么数据，不把这一格改成静默跳过。

记录上的文字画到页面上时要标出处：生成器经 `editmap.Origins` 登记「主键 + 字段路径」，
那个元素戴上 `data-e`，出处表写成 `<页目录>/edit.json`，就地编辑靠它找回那一格
（见 `.claude/rules/backend.md`《主键页：改记录上的一格》）。四处在标：`rows.expand()`
交出每格的出处、`convert-doc` 戴在 `<td>` 上；`render.py` 的 `Page.mark()`；神器模组页的
`.mod-desc`；护甲套装页的来源、标签与 `.bonus-body`。只标站内写的字段，标到 manifest
字段上当场中止。新加一处从记录取文字的渲染要一起标，不标的那一格线上改不了。
产出里读这些元素的正则要容得下属性（`<b class="r-tag"[^>]*>`），写成裸标签会被
`data-e` 打断。

源稿行里写的主键直接交给页面索引（`ROW_KEYS`），「异域 PERK」格里每个名字的主键同样由 `rows.py` 交出（`ROW_PERKS`），两者都不按名字反查：催化剂给的效果不在武器自己的插件池里，按名字查不到。索引里的图标路径一律相对站根，由 `pagedex.site_path()` 从 `<img src>` 换算。
...
另外两条：着色 span 不得嵌套（嵌套说明 `wrap()` 的整块判定被改坏了；`typeset()` 加的 `.cond` 包着着色 span 不算）；产出里不得有没转换的 `{` `}`。
...
「键：值」行携带结构信息，键名固定在 `META_KEYS`：`副标题`、`小标题`、`标题`、`本体图`、`徽章`、`括注`、`图标`、`标签`、`标签（站点补充）`。骨架展开时 `本体图` 取神器本体记录的 `icon_local`（`icons.py` 按「带 `derived.tiers`」认出神器、拉它的官方图），落在分节标题栏最左、徽章之前。每段说明走 `markup.typeset()`，与护甲模组页的 `说明排印：是` 同一份。这些行按整行剥离，剩下的即正文，所以正文里出现同名的键会被吃掉。`keys_in()` 逐块核对键行条数，数目不符即中止，不让内容悄悄消失。键行里的值必须占一行，其中的换行写成字面 `<br>`。
...
结构计数：分节 7、模组 147、图标引用 163、图标文件 133、span 开闭相等、残留内联样式 0、未转换的着色标记 0。分节没有模组也报错。
...
2. **计数断言**：分类 7、套装 56、效果 112、图标引用 112、每个套装恰好一条 2 件加一条 4 件。英文原表只给得出 110 枚（`N_TABLE_ICONS`），另 2 枚按 manifest 的效果图标补。
...
匹配那一侧要把中英之间那个排版空格允许回来（`items.pattern()`）：表的键归一化过，
源稿按 design.md 三节写「SUROS 政权」，拿键去字面匹配一个都对不上。
...
`INLINE` 是一趟正则走完的分支表，顺序即优先级：`**粗体**` → 反引号代码 → `“buff 名”`（走 `.term` 暖沙） → `[?]` 待测 → `[数值]` PvP → 数值位上的 `?` → `GUARD` 屏蔽段 → `GLOSSARY` 词表。引号在前保证 buff 名整体一个颜色，不会被词表再切一刀。`GLOSSARY` 内部必须长词在前（`能量球` 先于 `能量`、`威能弹药` 先于 `弹药`）。
...
```bash
python3 tools/convert-armor-sets.py --icons <英文原表导出.html>   # 从英文原表抽 110 枚
