...
**通过那一格的 slug 预填成「八位随机数-职业」**（`default_slug()`，职业从源稿的
...
| `类别：` | 强度或创意，单选必填。索引页按它分两个大节，详情页的铭牌接成「职业 · 元素 · 类别」。与 `定位` 是两回事——定位说这套配装在队伍里干什么，类别说它为什么被推荐 |
...
索引页 `builds/index.html` **按类别分两个大节（强度、创意），节内按职业分小节**。跳转
