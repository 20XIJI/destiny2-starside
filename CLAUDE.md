...
**类型只有五种**：新增、改版、订正、正名、补注，写别的词会破掉 `.act` 那一列的对齐。
**同一天之内按这个顺序排，同类型的连成一段**；类型内同一页的几条再挨在一起
（整页级那条在前，带「·」的条目级跟在后）。**类型与后面的链接之间不留空格**——`.act` 是定宽的
悬挂缩进标签，多一个空格就把正文推出那一列 4px。`{act|…}` 的类定义在 `changelog/style.css`，
`check_terms.py` 的 G3 会核。

| 类型 | 用在 | 写法要求 |
|---|---|---|
| 新增 | 新页面、新子页、新分节 | 子页写成「父页 · 子页」，两级都给链接 |
| 改版 | 结构与版式改动 | 改首页或外壳时，页面那一格写「首页」，不给链接 |
| 订正 | 数值改了 | 写清改成什么，不写「原为」 |
| 正名 | 名称改了 | 旧名与新名都用「」引起来 |
| 补注 | 补一条此前没写的说明 | 直接写那条说明，不写「补注了」 |

`条目` 那一段指向页面内的具体模组、首领或技能；整页级的改动省掉它，只留
「[页面](链接)：」。日期分节倒序，新的一天加在最上面。
