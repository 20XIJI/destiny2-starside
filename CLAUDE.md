...
**页面由九行面板组成，每一行拉满版心（1060px），行内的面板按 `--n`（自己的格数）分
...
| `核心：` | 必须等于本页的异域武器或异域护甲之一，决定页首那枚 96px 的图 |
...
| `六维：` | 六格、用 `｜` 隔开，顺序钉死（生命、近战、手雷、超能、职业、武器），值当纯文本落 |
...
的是分节标题——三个职业那三条正被头部的职业 chip 用着，删了那排 chip 就空了；
