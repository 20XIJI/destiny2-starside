...
```
mods.py            护甲模组变体：官方物品表 → tools/mod-variants.json + builds/icons/
...
`.entry` 形状。筛选交给工具条那只搜索框（卡片即 `data-item`），多标签 chip 筛选等配装
过二十套再做。

### 填表页

`builds/new/` 是给推荐人填的公开页：选完直接生成标准源稿，复制发过来即可挂站。
表单由 `builds/new/form.js` 按 `builds/vocab.js` 建出来——两千条选项写进 HTML 就是把
词表抄了第二份。`vocab.js` 与生成器查的是同一份词表，**填表页列得出来的名字，生成器
一定查得到**。
