...
一份稿子挂得越久，它起手那一版越可能已经被别人通过的改动顶掉。同一个人同一篇
只留一条待审，重提即改写，不堆第二份。

**`base` 存的是提交那一刻的正文**，审核页的 diff 与它比，不与当前正文比——同篇有
两份待审时，与当前比会把另一份的改动也算进这一份的 diff 里。**提交时带上起手那一版
的 hash，与库里当前值不同就当场拒**（`stale`）：照旧文改出来的稿子一旦通过，会把那条
已通过的改动整个回退掉，而它从来没落过盘，git 上一点痕迹都没有。

**结案即把两份全文换成那几行增删**（`diff` 字段，上限 8 KB，审核页现成算好原样带过来，
后端不再实现一遍 LCS）。正文已经进了 `docs`、跟着构建进了 git——**git 才是历史**，
库里再留一份 159 KB 的副本没有意义。留下的那一段答的是「谁在什么时候把哪一篇的哪几行
改成了什么」，「改动记录」那一页看的就是它。
...
等价关系由断言页钉住：**`npm run build` 过得去的 75 份源稿，前端闸门必须零错零提示**。
...
**通过时要定 slug，就地填。**预填成「八位随机串-职业」（职业从源稿的 `职业：` 行现读）：
这一格必填，而审的人多数时候不想在这里停下来想名字。slug 即文件名，也是点赞的 `_id`，
所以**形状与查重都验在云函数**：`^[a-z0-9][a-z0-9-]*$`、赛季形如 `s29-…`、已通过的投稿
与库里已有的源稿都不许撞。撞了当场拒，不会盖掉上一份。
