...
首行 `# 配装名`，其后是头部键（`推荐人`、`描述`、`更新`、`场景`、`定位`、`分支`、`核心`），
正文六个分节：`职业`、`武器`、`护甲`、`神器`、`六维`、`注解`。槽位一行一格，
**留空就是整行删掉**，不写占位符。

| 键 | 写法 |
|---|---|
| `推荐人：` | 一行一个人，`名字 \| 链接 \| 头像`，后两段可省。头像放 `builds/avatars/`，文件名取内容 md5 前 10 位 |
...
索引页 `builds/index.html` 按猎人、泰坦、术士三节列当前赛季的卡片，卡片复用首页那套
`.entry` 形状，左缘那条 2px 亮边跟着该配装的分支色走。筛选交给工具条那只搜索框（卡片即
`data-item`），多标签 chip 筛选等配装过二十套再做。
