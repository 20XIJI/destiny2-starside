...
| 动作 | 请求 | 落在哪 |
|---|---|---|
| 访问计数 | `POST {a:"hit",uv,p}` | `counters` 的 `pv:<日>`、`pv:total`、`uv:<日>`、`page:<路径>` |
| 读计数 | `GET ?a=stats` | 首页页脚那句「今日 X 次访问 · 累计 Y」 |
| 点赞 | `POST {a:"like",id,d}` / `GET ?a=likes` | `likes`，`_id` 是「赛季_slug」 |
| 投稿 | `POST {a:"sub",md}` | `subs`，`ok` 三档：0 待审、1 通过、-1 驳回 |
| 审核台读写 | `POST {a:"list"/"mark"/"pages",k}` | 要凭据，见下 |
...
计数就是一条 doc：`_id` 即键名，字段只有 `n`。**先 update 再 set**，doc 不存在时
才多一次往返；同一个键的首次并发写会互相覆盖成 1，掉几个数不值得上事务。

**同一个浏览器每小时只算一次访问**：按页面加载数记会把一个人翻十页记成十次。
窗口写在 localStorage 的两个定长键上（`sv` 存小时、`svd` 存天），不按小时各存
一个键。UV 按天首次；localStorage 不可用时去不了重，那台机器每次加载都算一次。
