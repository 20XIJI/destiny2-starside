...
`deploy.py`、`sync.py` 和两支索引生成器拒绝未知参数及选项缩写；`--help` 不进入业务操作。
部署的 `--check` 只能单独使用，`--prune` 必须配合 `--all`。
`--all --prune --dry-run` 显示模式、环境与挂载路径，但不查询远端额外文件，
因此不提供待删清单；真正发布时由 `tcb` 执行 prune。

同步的 `--seed` 不能与 `--mine/--theirs` 并用，同一 `_id` 不能同时选择两方向；
不同 ID 可以同次择向。完成项逐篇即时回执，失败不会回滚已完成项。
网络失败的写入结果可能未知，核对后再重跑；seed 的基线仍在全部成功后重记。
...
三个资料目标命令（`--normalize`、`--suggest`、`--apply`）都只接受裸 slug；
未知或排除的目标非零退出，不回退全量。术语闸门完整打印所有诊断，不截断到 60 条。
生成错误带当前源稿路径，表格结构错误带可确定的源稿行号，合集缺项带成员标题。

`convert-doc.py <slug>` 仅更新该资料页；`convert-build.py <slug>` 仅更新各赛季匹配的详情。
它们不刷新目录、首页、搜索及关联词表；配装词表仍读取已生成的资料页。
发布前运行 `npm run build`，单页命令不会隐式扩成全量构建。

### Excel 人工核对文件

```bash
python3 tools/json2xlsx.py sheet.json                 # 默认 sheet.xlsx，存在即拒绝
python3 tools/json2xlsx.py sheet.json reviewed.xlsx    # 另存
python3 tools/json2xlsx.py sheet.json --force          # 显式覆盖
```

输入与输出同文件（包括硬链接、符号链接别名）始终拒绝，`--force` 也不能绕过。
目录不能当输出文件，缺失的父目录须先手工创建。候选文件完整保存后才原子发布；
无 `--force` 时，预检后新出现的输出也不会被覆盖。
缺图时新目标可得到标为「部分结果（缺图）」的 xlsx，并退出 1；
已有目标即使带 `--force` 也保留原文件。帮助与输出安全预检不依赖 openpyxl。
...
**CSS 与 JS 上传时剥掉块注释。**`deploy.py` 的 `stage_one()` 把 `/* */` 换成等量换行再放进
暂存目录，源稿不动、行号不移；整站 `.css`/`.js` 从 824 KB gz 降到 692 KB，每页首屏省 37–41 KB。
字符串里含 `/*` 的文件整个跳过、原样发。
