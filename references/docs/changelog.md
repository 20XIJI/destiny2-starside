...
- {act|新增}[弹药生成机制](../ammo/index.html) · 弹药块拾取量：14 种武器每块给多少发
...
- {act|改动}编辑台：逐处审核并成整批事务，陈旧提案保留对照
- {act|改动}资料页：编辑目标改为按原文唯一定位
- {act|改动}[配装工具](../builds/new/index.html)：输入法确认与选择分开，导入改为每套写稿一次
...
- {act|改动}首页：搜索带右侧新增发电入口，署名后那枚换成菱形档位
...
- {act|新增}[技能冷却](../ability-cooldown/index.html)：113 个技能的冷却曲线与回复倍率
- {act|改动}[游戏机制](../game-mechanics/index.html) · 技能能量生成：整节归入[技能冷却](../ability-cooldown/index.html)
...
- {act|订正}[弹药生成机制](../ammo/index.html) · 注解：删去 200 属性额外弹药量那段观察
...
- {act|改动}首页：字标右侧加一枚三职业图标互换的标记
- {act|改动}首页：窄屏不再隐藏字标右侧那枚标记
- {act|改动}首页：刷新回到页首，不再还原上次的滚动位置
...
- {act|新增}[武器特性详解](../weapon-perks/index.html)：406 条特性、模组、框架固有与起源特性，带图标与全部数值
- {act|新增}[轮换速查表](../rotation/index.html) · RAID&地牢轮换：突袭与地牢的每周轮换，本周居中，前后各看两周
- {act|改动}首页：右上角加 QQ 群号与支持入口
- {act|改动}首页：加全站搜索，一个框搜 25 页 1979 个条目
...
- {act|订正}[异域武器详解](../exotic-weapon/index.html)：「电击」→「震颤」
