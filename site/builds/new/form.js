...
(function () {
  ...
  /* 源稿的形状（必填项、合集怎么切、套数上限）走 builds/source.js，着色标记的判读走
     admin/dialect.js：填表页、审核台、云函数与构建各自只认同一份。页面上两者排在这一份
     前面；Node 里（check_quality.cjs）按相对路径 require。 */
  var node = typeof module !== 'undefined' && module.exports;
  var B = node ? require('../source.js') : window.starsideSource;
  var D = node ? require('../../admin/dialect.js') : window.starsideDialect;
  ...
  if (typeof module !== 'undefined' && module.exports) {
    module.exports = { tidy: tidy, unmark: unmark, remark: remark, shiftSpans: shiftSpans };
  }
  ...
  function tabs() {
    ...
    ['set-copy', 'set-add'].forEach(function (k) {
      ...
      if (btn) btn.disabled = sets.length >= B.SET_MAX;
    });
  }
  ...
  function addSet(copy) {
    if (sets.length >= B.SET_MAX) return;
    sets[cur] = oneMd();
