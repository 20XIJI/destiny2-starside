...
# 一行人写说明照不到的地方。现存 209 个、24 组，分三类，全部是既有缺陷——索引里
# 只是把同一串 hash 存两遍，至今没有人交叉核对过。
#
...
#     适配框架 13、微型导弹框架 12、高冲击力框架 10，以及烈焰战锤、故我在、
#     英勇利刃。要治得让解析器认得出括注里写的是武器类型或元素，不是版本。
...
def build(page, facts):
    ...
    for entry in entries:
        ...
        for key in keys:
            ...
            # 图标那一格可能引到好几张图（{ico2|} / {ico4|}），实体只记第一张——
            # 它是这件东西自己的图，后面那些是同一行里别的东西的。
            mine = research.icon_paths(research.field(entry, 'icon'))
            if src or mine:
                got['icon'] = {'source': src or None,
                               'file': '%s/%s' % (page, mine[0]) if mine else None}
