...
def artifact_mods(F):
    ...
    名字、图标、归属与说明现读神器模组页的索引：同一枚模组可以挂在几件神器上（群敌
    飞梭在好奇之器、废墟石板与女王兰香炉上是同一个 hash），记录上的 site_artifact
    只记得一件；图标由那一页源稿的「图标」行定，可以与记录上的官方图不同（乘胜追击）；
    说明用那一页渲染好的一份，那一页的方言不开富文本（见 CLAUDE.md），这里再渲染一遍
    会各渲各的。
    """
    import pagedex
    arts, art_at, mods, at = [], {}, [], {}
