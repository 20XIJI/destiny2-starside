...
CLASSES = ('猎人', '泰坦', '术士')
# 配装的三个分类轴。顺序即工具条上的顺序，也是编辑台左栏的分组顺序。
#
# 场景说这套配装在哪个活动里用。**一套配装只有一个场景**，唯一的例外是
# CO_SCENES：突袭与地牢的敌人、机制与配装取舍高度重合，同一套两边都能打是常态，
# 强行二选一会让作者随手挑一个，那一维就废了。别的组合不放行——「哪都能用」在
# 这套轴里不是一个值：它由「没被更高难的场景占住」自然表达，写成第七个值会让
# 读者点任何一个具体场景时都漏掉它。
SCENES = ('突袭', '地牢', '宗师/终极', '日常', '功能性', 'PVP')
CO_SCENES = frozenset({'突袭', '地牢'})
# 「功能性」由审核员指定：它与标签「机制」的边界要看正文才判得出，交给投稿人勾
# 会长出两条录入路径。填表页照样出这一枚，但默认藏着，由 form.js 的 review() 在
# 审核态掀开——**藏而不是不出**：不出的话 pressTags() 找不到按钮，审核台打开一份
# 已是「功能性」的稿子再存回去，那一行会被 sumTags() 重算成空。
REVIEW_SCENES = frozenset({'功能性'})

# 强度说这套配装凭什么被推荐，单选。「创意」不是「弱」——它是「价值在玩法体验，
# 别拿强度评我」，作者的这个诉求是真的，占一档。
TIERS = ('meta', '强力', '创意')

# 标签说这套配装在队伍里干什么，多选、可不写，**取值随场景**。
# 宗师/终极、日常、功能性三个场景没有标签：那几处的配装不按分工挑。
# 一处定义，四处消费：tags_of() 的闸门、填表页那一排、索引页的筛选、审核台。
_RAID_TAGS = ('输出', '机制', '五壮举')
